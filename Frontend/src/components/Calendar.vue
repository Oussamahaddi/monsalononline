        ...
        <div class="w-fit">
            ...
            <div class="bg-white w-fit flex flex-col items-center  rounded-lg shadow-[0_0_4px_black]">
                ...
                <div class="grid grid-cols-7 text-center p-2 gap-4 border-t border-t-gray-400 ">
                    ...
                    <p @click="selectedDay(date), visible()" v-for="date in lastDateOfMonth" :class="toDay(date), hidePassedDay(date)" class="border rounded-xl cursor-pointer">{{ date }}</p>
                </div>
            </div>
        </div>
        <div :class="hidden">
            <h2 class="text-center my-4 font-semibold">{{ currentDay }} {{ currentMonthName }}</h2>
            <ul class="grid gap-4 grid-cols-2">
                <li v-for="n of times" :id="n" class="li" @click="handleClick(n)">{{ 10+n-1 + ':00' }} - {{ 10+n + ':00'}}</li>
            </ul>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        hidePassedDay(date) {
            ...
            return date < toDay && this.currentMonthNumber ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "";
        },
        selectedDay(date) {
